...
//		while(from < to) {
	...
	private static void asc(int[] ary) {
		int len = ary.length;
		for (int i = 1; i < len; i++) {
			int key = ary[i];
			int index  = search(ary, key, 0, i - 1);
			for (int j = i; j > index; j--) {
				ary[j] = ary[j - 1]; // j-1 最小等于index
			}
			ary[index] = key;
		}
		System.out.println("\n-----------升序");
	...
	private static int search(int[] ary, int key, int left, int right) {
		while (left < right) {
			int mid = (left + right) >>> 1;
			if (ary[mid] < key) {//mid=4, key=5
				left = mid + 1;
			} else {
				right = mid - 1;
			}
